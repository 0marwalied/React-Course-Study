let notes = [
  ...
  `The <code>then()</code> method is used to specify what should happen when a Promise is resolved successfully. It takes two optional arguments: a callback function for the resolved case and another for the rejected case.`,
  `The <code>catch()</code> method is used to handle errors or rejections in a Promise chain. It is called when a Promise is rejected and allows you to define how to respond to the error.`,
  `The <code>finally()</code> method is used to execute code after a Promise has been settled, regardless of whether it was resolved or rejected. It is often used for cleanup actions.`,
  ...
  `We can write Promise with setTimeout with this two styles <code>(short hand)</code>:<br>
  <br>
